...
func InitGmailClient() (*gmail.Service, error) {
...
func getClient(config *oauth2.Config) *http.Client {
	...
	// time.
	tokFile := "configs/gmail_token.json"
	...
	// if the token is expired, refresh it using refresh_token
	if !tok.Valid() {
		updatedToken, err := config.TokenSource(context.Background(), tok).Token()
		if err != nil {
			fmt.Println(err)
			log.Fatalf("Refreshing token failed")
		}
